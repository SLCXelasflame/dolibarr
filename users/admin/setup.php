...
/**
 * \file    users/admin/setup.php
 * \ingroup users
 * \brief   Users setup page.
 */
...
// Translations
$langs->loadLangs(array("admin", "users@users"));

// Initialize technical object to manage hooks of page. Note that conf->hooks_modules contains array of hook context
$hookmanager->initHooks(array('userssetup', 'globalsetup'));
...
$item = $formSetup->newItem('USERS_MYPARAM1');
$item->fieldOverride = (empty($_SERVER['HTTPS']) ? 'http://' : 'https://') . $_SERVER['HTTP_HOST'];
$item->cssClass = 'minwidth500';

// Setup conf for selection of a simple string input
$item = $formSetup->newItem('USERS_MYPARAM2');
$item->defaultFieldValue = 'default value';
$item->fieldAttr['placeholder'] = 'A placeholder here';

// Setup conf for selection of a simple textarea input but we replace the text of field title
$item = $formSetup->newItem('USERS_MYPARAM3');
$item->nameText = $item->getNameText().' more html text ';

// Setup conf for a selection of a thirdparty
$item = $formSetup->newItem('USERS_MYPARAM4');
$item->setAsThirdpartyType();

// Setup conf for a selection of a boolean
$formSetup->newItem('USERS_MYPARAM5')->setAsYesNo();

// Setup conf for a selection of an email template of type thirdparty
$formSetup->newItem('USERS_MYPARAM6')->setAsEmailTemplate('thirdparty');

// Setup conf for a selection of a secured key
//$formSetup->newItem('USERS_MYPARAM7')->setAsSecureKey();

// Setup conf for a selection of a product
$formSetup->newItem('USERS_MYPARAM8')->setAsProduct();
...
$formSetup->newItem('USERS_MYPARAM9')->setAsSelect($TField);

// Setup conf for a multiselect combo list
$item = $formSetup->newItem('USERS_MYPARAM10');
$item->setAsMultiSelect($TField);
$item->helpText = $langs->transnoentities('USERS_MYPARAM10');

// Setup conf for a category selection
$formSetup->newItem('USERS_CATEGORY_ID_XXX')->setAsCategory('product');

// Setup conf USERS_MYPARAM10
$item = $formSetup->newItem('USERS_MYPARAM10');
$item->setAsColor();
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/users/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if ($filefound) {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=users-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'USERS_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'USERS_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-users page-admin');
...
$head = usersAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "users@users");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("UsersSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'USERS_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'USERS_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
